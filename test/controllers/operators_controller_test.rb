...
class OperatorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create operator" do
    assert_difference('Operator.count') do
      post operators_url, params: { operator: { name: @operator.name, permission: @operator.permission, status: @operator.status, telphone: @operator.telphone } }
    end
  ...
  test "should update operator" do
    patch operator_url(@operator), params: { operator: { name: @operator.name, permission: @operator.permission, status: @operator.status, telphone: @operator.telphone } }
